...
(setq completion-styles '(initials partial-completion flex basic)
      read-filename-completion-ignore-case t
      completion-auto-help nil
      completion-auto-select nil 
      completion-cycle-threshold 10 ;; always cycle through completion candidates
      completions-format 'one-column ;; completion list buffer format
      completions-sort nil ;; sort candidatate alphabetically
      completions-max-height nil ;; no height limit for completion list buffer
...
;; with active region typing character inserts it without replacing regions
(delete-selection-mode -1)

(setq set-mark-command-repeat-pop t ; C-u C-SPC C-SPC... keeps popping local mark
	  mark-ring-max 512
...
(use-package emacs
  :hook (occur-mode . follow-mode)) 	; auto enable follow mode
