...
(defalias 'my-grep-map
  (let ((map (make-sparse-keymap)))
    (define-key map (kbd "g") 'grep)
    (define-key map (kbd "f") 'grep-find)
    (define-key map (kbd "r") 'rgrep)	; interactive grep-find
    (define-key map (kbd "z") 'zgrep)
    map)
  "Grep")
...
(defalias 'my-lsp-map
  (let ((map (make-sparse-keymap)))
    ...
    (define-key map (kbd "e") 'eglot-events-buffer)
    (define-key map (kbd "f") 'eglot-format)
    (define-key map (kbd "r") 'eglot-rename)
    map)
  "LSP Eglot")

(defalias 'my-embark-map
  (let ((map (make-sparse-keymap)))
    (define-key map (kbd "a") 'embark-act)
    (define-key map (kbd "b") 'embark-become)
    map)
  "Embark")

(defalias 'my-window-map
  (let ((map (make-sparse-keymap)))
    (define-key map (kbd "b") 'my-split-below)
    (define-key map (kbd "d") 'my-toggle-window-dedication)
    (define-key map (kbd "o") 'my-other-window)
    (define-key map (kbd "r") 'my-rotate-windows)
    (define-key map (kbd "t") 'my-toggle-window-split)
    map)
  "Window")
