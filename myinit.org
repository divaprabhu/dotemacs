...
#+begin_src emacs-lisp :tangle no  
  (use-package eca
    ...
    (:map my/ai-tool-prefix-map
    	...
    	("a" . eca-chat-tool-call-accept-next)
    	("r" . eca-chat-tool-call-reject-next))
    )
#+end_src
** Agent Shell
#+begin_src emacs-lisp :tangle no  
  (use-package agent-shell
    ...
    ((claude-agent-acp . "bun add -g @agentclientprotocol/claude-agent-acp"))
    :defer t
    :init
    (setenv "ANTHROPIC_AUTH_TOKEN" (auth-source-pick-first-password :host "ollama.com"))
    (setenv "PATH" (concat "~/.bun/bin:~/.local/bin:" (getenv "PATH")))
