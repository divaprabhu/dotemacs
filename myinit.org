...
| *DOWN* OR *C-n*    |     | next-line-or-history-element                                               |
|                |     | In multiline minibuffer, go to next line otherwise go to next history)     |
| *UP* OR *C-p*      |     | previous-line-or-history-element                                           |
...
|-----+--------------------------|
- Display keybinding suggestion for 5s instead of default 2s and show shorthand
  #+begin_src emacs-lisp
    (use-package simple
      :ensure nil
      :custom
...
| *C-h o*            |   | describe-symbol (combination of function, variable, face) |
...
- In emacs backspace is indicated with DEL
  ...
  |----------------------------+------------+----------------------------------------------------|
  |----------------------------+------------+----------------------------------------------------|
  | *C-u C-SPC [C-SPC C-SPC...]* | *`m*         | jump to last local mark                            |
...
| *M-z*     |    | zap-to-char                                                         |
|         |    | (kill till next occurrence of char including char)                  |
|---------+----+---------------------------------------------------------------------|
** Yanking
  |-----------------+------+-----------------------------------------------------------------|
...
* Display
** Scrolling
- In emacs scroll up means text moves up and scroll down means text moves down
...
- Highlight change mode - Minor mode that highlights part of buffer that were changed recently
...
- Line truncation only display till what fits on screen. An arrow is
  ...
  |--------------------------------------+---------------------------------------------------------|
  | C-x x t or M-x toggle-truncate-lines | toggle line truncation                                  |
  ...
  |                                      | visual-line-mode disables truncate-lines and vice versa |
...
| *M-e* or *M-s e* |     | isearch-edit-string                              |
| *M-e C-f*      | *C-w* | add search string characters following point     |
|--------------+-----+--------------------------------------------------|
** Isearch Yanking
|---------+-----------------------------------------------------------------|
...
- they don't default for lax match like in search
  ...
  |                                                 |                       | The query replace versions prompt for each match whereas         |
  |                                                 |                       | other replace replaces all instances                             |
  ...
  | N                                               |                       | skip to next buffer with no other replacement in current buffer  |
  | ? or C-h                                        |                       | display help                                                     |
  |-------------------------------------------------+-----------------------+------------------------------------------------------------------|
...
| *C-g C-/*             | *C-r* | redo                                                               |
| *C-?*                 |     | undo-redo (redo last ARG changes)                                  |
...
** Variations
|-----------+-----------------------------------------------------------------------|
| C-x C-k q | kbd-macro-query (this key binding is entered during macro definition) |
...
- Emacs auto saves during fatal error like network disconnect or when killed
  ...
  |                                    | Typing = or diff at prompts shows the diff between two files        |
...
- Editing patch in diff mode tries to maintain line number so the patch is still valid
  ...
  | *M-p*                                 | diff-hunk-prev                                                          |
  | M-}                                 | diff-file-next                                                          |
  |                                     | Move to next file start in multi-file patch Move to previous file-start |
  | M-{                                 | diff-file-prev                                                          |
  | *M-k*                                 | diff-hunk-kill                                                          |
  | M-K                                 | diff-file-kill (in multi-file patch kill current file part)             |
  | *C-c C-a*                             | diff-apply-hunk (apply hunk to target file)                             |
  |                                     | with prefix, revert hunk                                                |
  | *C-c RET a*                           | diff-apply-buffer (apply all hunks in the buffer)                       |
  | *C-c C-b*                             | diff-refine-hunk (highlight char/words that are different in the line)  |
  | *C-c C-c*                             | diff-goto-source (goes to new version)                                  |
  |                                     | use prefix to go to old version                                         |
  |                                     | with vc, jump to current working version                                |
  |                                     | with prefix, jump to old revision                                       |
  ...
  | *C-c C-s*                             | diff-split-hunk (split hunk at point, works only in unified diff))      |
...
- Opening image selects Image Mode
  ...
  | RET                          | image-toggle-animation (play or pause animation)                  |
  | image-animate-loop           | if non-nil, play animation in loop                                |
  | f                            | image-next-frame (of animation)                                   |
  | b                            | image-previous-frame                                              |
  | i + +...                     | image-increase-size (of animation)                                |
  | i - -...                     | image-decrease-size                                               |
  | i v                          | image-flip-vertically                                             |
  | i h                          | image-flip-horizontally                                           |
  | i r r...                     | image-rotate                                                      |
  | i o                          | image-save (prompts file name to save)                            |
  | i c                          | image-crop (works only if external program is available)          |
  | image-crop-crop-command      | defaults to imagemagick convert                                   |
  | i x                          | image-cut (same as crop but fill removed part with color)         |
  | image-crop-cut-command       | defaults to imagemagick covert                                    |
  | image-cut-color              | default color to fill                                             |
  | imagemagick-enabled-types    | image types rendered using ImageMagick                            |
  |                              | setting to t uses ImageMagick for all images                      |
  | imagemagick-types-inhibit    | image types that should never be rendered using ImageMagick       |
  |                              | setting to t, disable ImageMagick entirely                        |
  | image-use-external-converter | if t, use external converters to convert image to png and display |
  | image-converter              | external program which should be used for converting unsupported  |
  |                              | image type                                                        |
  | image-convert-to-format      | format to which converter should convert the image to render      |
  |------------------------------+-------------------------------------------------------------------|
** Configuration
#+begin_src emacs-lisp
...
- At any time only one buffer is selected which is called current
  buffer
- Buffer local variables are used to store buffer specific information
  like file it is visiting, major and minor modes, whether modified etc
- For a 64 bit machine, max buffer size is 2^{61} -2 bytes. In
  addition system memory also limits buffer size
...
| {        |          | narrow current column width by prefix characters         |
| }        |          | widen current column width by prefix characters          |
| I        |          | Toggle display of internal buffers                       |
|----------+----------+----------------------------------------------------------|
** Other options
|----------------------------------+---------------------------------------------|
...
- Tab Bar is displayed at the top of frame and different from tabline
...
** Introduction
  - Internally, Emacs uses its own multibyte character encoding,
    which is a superset of the “Unicode” standard. This internal
    encoding allows characters from almost every known script to be
    intermixed in a single buffer or string. Emacs translates between
    the multibyte character encoding and various other coding systems
    when reading and writing files, and when exchanging data with
    subprocesses.
  |-----------+------------------------------------------------------------|
  | C-h h     | view-hello-file                                            |
  |           | displays etc/hello which shows shello in various languages |
  | C-x 8 RET | insert-char                                                |
  | C-x 8 [   | short hand for ‘ (left single quotation mark)              |
  |           | there are other similar shorthand                          |
  | C-x =     | what-cursor-position (show information about char at point |
  | C-u C-x = | describe-char (shows detailed description)                 |
  |-----------+------------------------------------------------------------|

** Language Environments
  - The language environment controls which coding systems to
    recognize when reading text (*note Recognize Coding::).  This
    applies to files, incoming mail, and any other text you read into
    Emacs.  It may also specify the default coding system to use when
    you create a file.  Each language environment also specifies a
    default input method.
  |--------------------------------+-----------------------------------------|
  | C-h L LANG-ENV RET             | describe-language-environment           |
  | 'current-language-environment' | show current language environment       |
  | M-x set-language-environment   | change current lang env globally        |
  | 'language-info-alis'           | list of supported language environments |
  |--------------------------------+-----------------------------------------|
  - Emacs honours the LC_ALL, LC_CTYPE and LANG env variables
** Input Methods
  - An “input method” is a kind of character conversion designed
    specifically for interactive input.
  - In Emacs, typically each language has its own input method;
    sometimes several languages that use the same characters can share
    one input method.  A few languages support several input methods.
  - The simplest kind of input method works by mapping ASCII letters
    into another alphabet; this allows you to use one other alphabet
    instead of ASCII.  The Greek and Russian input methods work this
    way.
  - A more powerful technique is composition: converting sequences of
    characters into one letter.  Many European input methods use
    composition to produce a single non-ASCII letter from a sequence
    that consists of a letter followed by accent characters (or vice
    versa).  For example, some methods convert the sequence ‘o ^’ into
    a single accented letter.  These input methods have no special
    commands of their own; all they do is compose sequences of
    printing characters.
  - Sometimes it is useful to cut off input method processing so that
    the characters you have just entered will not combine with
    subsequent characters.  For example, in input method
    ‘latin-1-postfix’, the sequence ‘o ^’ combines to form an ‘o’ with
    an accent.  What if you want to enter them as separate characters?
    - One way is to type the accent twice; this is a special feature
      for entering the separate letter and accent.  For example, ‘o ^
      ^’ gives you the two characters ‘o^’.
    - Another way is to type another letter after the ‘o’--something
      that won't combine with that--and immediately delete it.  For
      example, you could type ‘o o <DEL> ^’ to get separate ‘o’ and
      ‘^’.
    - Another method, more general but not quite as easy to type, is
      to use ‘C-\ C-\’ between two characters to stop them from
      combining.  This is the command ‘C-\’ (‘toggle-input-method’)
      used twice.
  - Input methods are inhibited when the text in the buffer is
    read-only for some reason.  This is so single-character key
    bindings work in modes that make buffer text or parts of it
    read-only, such as ‘read-only-mode’ and ‘image-mode’, even when an
    input method is active.
  |-------------+------------------------------------------------------------------------|
  | C-x 8 <RET> | insert-char (insert char based on unicode name or code point)          |
  | C-x 8 e e   | emoji-insert (navigate emoji categories and choose one)                |
  | C-x 8 e l   | emoji-list (pop  a new buffer list all emoji. RET or click inserts it) |
...
** Selecting an Input Method
  ...
  |                          | With prefix, prompt for an input method with recently used selected as default |
  | C-x RET C-\ METHOD <RET> | set-input-method for the buffer                                                |
  | C-x \ METHOD <RET>       | activate-transient-input-method                                                |
  |                          | Enabled only for next single input character                                   |
  | *C-h I METHOD <RET>*       | describe-input-method                                                          |
  | C-h C-\ METHOD <RET>     | shows keyboard layout with ascii to input method translation and keybindings   |
  | M-x list-input-methods   |                                                                                |
  |--------------------------+--------------------------------------------------------------------------------|
** Coding Systems
  - Emacs uses it's own coding system internally. It converts from
    other systems to it's own system when reading and converts to
    other coding systems when writing. Conversion can also happen when
    sending receiving from terminal and exchanging data with
    subprocess
  |----------------------------------------------+-----------------------------------------|
  | C-x <RE> c CODE <RET> C-x C-f FILENAME <RET> | universal-coding-system-argument        |
  |                                              | set coding system for next command      |
  | C-h C CODING <RET>                           | describe-coding-system                  |
  | C-h C <RET>                                  | describe-coding-system currently in use |
  | M-x list-coding-systems                      | list all supported coding systems       |
  | M-x find-file-literally                      | Open file literally with no conversion  |
  |----------------------------------------------+-----------------------------------------|
  - Each of listed coding systems have 3 variants, which specify the
    end-of-line conversion
  |-------+-------------------------------------------------------------------------|
  | -unix | don't do any conversion, assume newline to separate lines. Used on Unix |
  | -dos  | assume carriage return followed by linefeed to separate lines           |
  |       | do appropriate conversion. Used on Windows                              |
  | -mac  | assume carriage return to separate lines                                |
  |       | do appropriate conversion. Used for Classic mac                         |
  |-------+-------------------------------------------------------------------------|
  - Coding system unix, dos and mac are aliases for undecided-unix,
    undecided-dos and undecided-mac. The character encoding is deduced
    from file text itself
  - The coding system ‘raw-text’ is good for a file which is mainly
    ASCII text, but may contain byte values above 127 that are not
    meant to encode non-ASCII characters.  With ‘raw-text’, Emacs
    copies those byte values unchanged, and sets
    ‘enable-multibyte-characters’ to ‘nil’ in the current buffer so
    that they will be interpreted properly.  ‘raw-text’ handles
    end-of-line conversion in the usual way, based on the data
    encountered, and has the usual three variants to specify the kind
    of end-of-line conversion to use.
  - In contrast, the coding system ‘no-conversion’ specifies no
    character code conversion at all--none for non-ASCII byte values
    and none for end of line.  This is useful for reading or writing
    binary files, tar files, and other files that must be examined
    verbatim.  It, too, sets ‘enable-multibyte-characters’ to ‘nil’.
  - The coding system ‘emacs-internal’ (or ‘utf-8-emacs’, which is
    equivalent) means that the file contains non-ASCII characters
    stored with the internal Emacs encoding.  This coding system
    handles end-of-line conversion based on the data encountered, and
    has the usual three variants to specify the kind of end-of-line
    conversion.
** Recognizing Coding Systems
  - Emacs tries to recognize coding system when reading text (file,
    subprocess etc)
  - Some coding systems can be automatically recognized using byte
    sequence but not all
  - When it can't recognized, it is done based on the priority defined
    in the selected language environment
  - The priority can be altered by M-x prefer-coding-system
  - Priority can include both coding system and end of line
  - File name can be used to indicate coding system via
    file-coding-system-alist. This can be updated using
    modify-coding-system-alist
  - Emacs automatically does eol conversion based on file contents.
    You can inhibit this conversion setting inhibit-eol-conversion to
    non-nill. For example, DOS-style files will display ^M characters
  - The auto-coding-alist and auto-coding-regexp-alist takes highest
    priority when specifying coding list even overriding coding tags
    specified in file
  - Another way is to use variables auto-coding-functions. This does
    not override coding tags specified in file

** Specifying a File's Coding System
  - Change the file's coding system with
    revert-buffer-with-coding-system
  - Coding system for a file can be specified in the file using
    -\star-...-\star- at the beginning or a local variable list at the end(file
    variables)
    Ex: -\star- coding: latin-1; -\star-
** Choosing Coding Systems for Output
  - Coding system is stored in 'buffer-file-coding-system'
  - If Emacs cannot characters entered using
    buffer-file-coding-system, it tries to find most-preferred coding
    system set by prefer-coding-system or language environment
  - Otherwise it will display list of coding system to chose
  |--------------------------+--------------------------------|
  | C-x <RET> f              | set-buffer-file-coding-system  |
  | M-x prefer-coding-system | set the coding system priority |
  |--------------------------+--------------------------------|
...
** Coding System for File Names
  ...
  |                          | No effect on reading/writing file contents |
  |                          | sets file-name-coding-system               |
  | M-x recode-file-name     | change file name's coding system           |
  |--------------------------+--------------------------------------------|
** Coding Sytems for X Keyboard Input
  |-------------------------+------------------------------------------------------|
  | 'x-input-coding-system' | sets the coding system to decode keyboard input from |
  |                         | input methods under X Window System                  |
  |-------------------------+------------------------------------------------------|
** Coding System for Terminal I/O
  |--------------------------+----------------------------------------------|
  ...
  |--------------------------+----------------------------------------------|
** Fontsets
  - Fontset is a collection of fonts
  - It is defined by list of font specifications
  - Fonts and font names are at system level but fontset is defined
    within emacs
  - Fontset can be used in place of a font within emacs
  - Emacs creates 3 fontsets by default:
  - Default fontset has wide variety of fonts and is default
    fallback. The -fn option to emacs sets the fontset
  - M-x describe-fontset to display info on specific fontset
  - fontset-default is the fontset used in current session
  - 'face-ignored-fonts' to ignore certain fonts
** Charsets
  - Emacs supports popular character sets like ascii, iso-8859-1,
    unicode etc
  - Language Environment also dictates the charset selection
  |----------------------------+------------------------------------------------------|
  | M-x list-charset-chars     | prompt charset name and display all characters in it |
  | M-x describe-character-set | displays information about a charset                 |
  |----------------------------+------------------------------------------------------|
* Modes
...
|------------------+------------------------------------|
  - Major Mode to open when a file is visited by specifying the first
    line something like: ; -*- mode: Lisp -*- We can also specify
    expression using eval ; -*- mode: Lisp; eval: (auto-fill-mode 1);
    -*-
  - It then uses .dir-locals.el facility to check if any file type
    specific settings
  - If the file contents begin with #!, it checks the interpreter
    specified in this line and uses this knowledge to set mode
  - In case of #! on first line, emacs also checks second line for -*-
    line
Once a major mode is found, emacs checks 'major-mode-remap-alist' to
check if mode has been remapped. If it is remapped, it will use the
remapped mode.
Ex: (add-to-list 'major-mode-remap-alist '(c-mode . c-ts-mode))
|-----------------+----------------------------------------------|
...
|         |         | if region is active, indent all lines in it               |
...
- Indentation inserts shortest possible series of tab and space to align to desired column
  |--------------------------+------------------------------------------------------------|
  | 'tab-width'              | distance between tab stops                                 |
  ...
  |                          | else insert tab                                            |
  | 'tab-first-completion'   | nil - commplete                                            |
  |                          | eol - only complete if point is at eol                     |
  |                          | word - complete unless next char has word syntax           |
  |                          | word-or-paren - complete unless next char is part of word  |
  |                          | or paren                                                   |
  |                          | word-or-paren-or-punct - complete unless next char is part |
  |                          | of word, paren or puncutation                              |
  |                          | Typing tab second time always completes                    |
  | M-x electric-indent-mode | automatically indent line after every RET                  |
  |--------------------------+------------------------------------------------------------|
** Code Alignment
|--------------------------------+---------------------------------------------------------------|
...
| M-x align-current              | similar to M-x align except it operates only                  |
|                                | on the section containing point irrespective                  |
|                                | of the current region                                         |
| 'align-region-separate'        | defines section boundaries in region used                     |
|                                | by align-current                                              |
|                                | 'entire' - M-x align behaves like align-entire                |
| M-x align-entire               | disregard align-region-separate and aligns entire             |
|                                | region as single section                                      |
| M-x align-regexp               | prompt for a regexp and align accordingly                     |
|                                | By default, this command adjusts the whitespace               |
|                                | that matches the first sub-expression of the regular          |
|                                | expression you specify.  If you call ‘M-x align-regexp’       |
|                                | with a prefix argument, it also prompts you for the           |
|                                | sub-expression to use and lets you specify the amount of      |
|                                | whitespace to use as padding, as well as whether to apply the |
|                                | rule repeatedly to all matches of the regular expression      |
|                                | in each line.                                                 |
|--------------------------------+---------------------------------------------------------------|
** Configuration
#+begin_src emacs-lisp
...
- “Filling” text means breaking it up into lines that fit a
  specified width.
- fill prefix allows para to be filled such that each line starts with
  special string
  |------------------------------+------+------------------------------------------|
...
- uses C-c prefix
- makes part of the buffer invisible
- when a line is invisible, it is replaced by ellipsis on the previous
  line
- multiple consecutive hidden lines produce just one ellipsis
- C-n, C-p etc treat ellipsis as part of previous line
- Killing ellipsis kills all following invisible text
*** Outline Minor Mode
- buffer local minor mode that behaves same as outline mode but can be
  used in conjunction with other modes
- While outline mode provides C-c prefix, the minor mode provides C-c
  @ prefix. It can be changed via outline-minor-mode-prefix variable
*** Outline Format
...
- any line which is not a heading line, is a body line. Body lines
  belong to preceding heading line
- A heading line with all following body forms an *entry*; heading
  with all subheadings and their body is a *subtree*
- 'outline-regexp' can be used to specify criteria for heading line
*** Outline Motion
|---------+----------------------------------|
...
|---------+-----------------------------------------------------------------------------|
| *C-c C-c* | outline-hide-entry (make body of current heading invisible)                 |
| *C-c C-e* | outline-show-entry (make body of current heading visible)                   |
| *C-c C-d* | outline-hide-subtree (make entire subtree(both sub-heading and their body)  |
|         | of heading invisible)                                                       |
| *C-c C-s* | outline-show-subtree                                                        |
| *C-c C-l* | outline-hide-leaves (hide everything except show heading and subheadings)   |
| *C-c C-k* | outline-show-branches (reveal subheadings if they were hidden)              |
| *C-c C-i* | outline-show-children (reveal only direct subheadings (one level)           |
| *C-c C-o* | outline-hide-other (hide everything except current heading and its parents) |
| *C-c / h* | outline-hide-by-heading-regexp (prompt regexp and hide any body lines of    |
|         | headings that match)                                                        |
| *C-c / s* | outline-show-by-heading-regexp (reveal bodies of headings matching regexp)  |
|---------+-----------------------------------------------------------------------------|
- isearch reveals body that is hidden if there is a match
- this behavior can be toggled with M-s i
| 'search-invisible'      | can be used to control the default behavior     |
| 'outline-default-state' | controls what headings will be visible after    |
|                         | outline mode is turned on                       |
|                         | non-nil - some headings are outlined            |
|                         | number - show heading up to that level          |
|                         | outline-show-all - no hiding                    |
|                         | outline-show-only-headings - show only headings |
|-------------------------+-------------------------------------------------|
...
|         | only level-1 heading, body, and level 2 headings are visible            |
|         | To looks under level 2 heading, place the cursor on it and type C-c C-z |
|         | numeric prefix exposes that many levels of children                     |
|         | M-0 C-c C-z whole subtree is expanded                                   |
| C- C-x  | foldout-exit-fold                                                       |
|         | hides all text and subheadings under the top-level heading and returns  |
|         | to previous view of buffer                                              |
|         | negative argument cancels that many levels of  narrowing without hiding |
...
|-----------+---------------------------------------------+-------------------------------|
| Subtree   | Heading + all descendants (headings + body) | Siblings, parent              |
| Entry     | Heading + its own body text                 | Child headings and content    |
| Children  | Direct child headings (depth+1 only)        | Grandchildren, body text      |
| Leaves    | Body/prose text blocks                      | Heading lines                 |
| Branches  | Heading lines within a subtree              | Body/prose text               |
| Sublevels | Headings beyond depth N (used with C-q)     | Body text, shallower headings |
|-----------+---------------------------------------------+-------------------------------|
...
|       | mark at end and point at beginning                                       |
|       | good to kill defun                                                       |
|       | marks comments at the beginning if they are not separated by newlines    |
| *M-a*   | backward-sentence (beginning of current or preceding sentence)           |
| *M-e*   | forward-sentence                                                         |
|       | sentence is usually a complete language construct smaller than defun but |
|       | larger than sexp such as variable definition or initialization or        |
...
- VC is also enabled in Dired buffers. Marked files or file on current
  line are treated as fileset
...
- VC Directory buffer allows viewing the version control status of
  ...
  |                               | (marks edited, added and removed files)                    |
...
- We can delete and rename files
  ...
  |                    | prompt for file name from working tree and stage       |
...
- Preparing patches
  |---------------------------------+-------------------------------------------------------------------------------|
  | M-x vc-prepare-patch            | prompt for revision(s) (comma separated) and email address to share the patch |
  | 'vc-prepare-patches-separately' | t - prepare and display message for each revision                             |
  |                                 | nil - generate a single message with all patches                              |
  |---------------------------------+-------------------------------------------------------------------------------|
- VC Options
  |------------------------+---------------------------------------------------|
...
:CUSTOM_ID: clone
:END:
You decide you want to hack on a repo. There's no vc equivalent to
=git clone...=. So you drop to the command line to do it. OR you write a
command, this one is mine:
...
which is more or less equivalent to Magit's status window. It also uses
plumbing from =vc-git=, the package to which most commands dispatch
their work for git repositories.

Commands in VC are under the prefix =C-x v=[[#fn-2][[2]]]. For Example,
you can invoke =vc-dir= at any point using =C-x v d= (personally, I
always use =project-vc-dir=, because I use =project.el=. The binding is
=C-x p v=).
...
:CUSTOM_ID: vc-dir
:END:
We'll focus first on this window, which gives an overview of the
repository status. Here is how the =vc-dir= buffer for my website repo
looks for this post:
...
- "VC backend" tells us what's the underlying source control solution.
  In this case, git.
- You can see the local branch name, remote branch, etc.
- Yes, there is support for stashes. In the =vc-dir= buffer, the stash
  commands are under =z=. Use =z ?= to see all the bindings.

The "unregistered" file is this post. The next logical action for a file
happens with the key =v=, which in this case, is to add it to the repo.
So I move point to the file and press the key:
...
You can also mark several files to operate on them at the same time,
using =m=. Pressing =M= will mark all files with the same status as the
current one, for example if I put point in any of the files I edited:
...
And then with =u= or =U= you can unmark individual files or remove all
marks, respectively.

There are many commands that depend on selected files, or point
position. For example =vc-revert= (not bound by default, I put it in
=k=), shows a diff of the changes and asks for confirmation to revert.
If files are selected, it will operate only on those. If there's no
selection and point is over any file, it will instead offer to revert
that single file. With point in the header, we can revert *all changes*
in one go.\\
Other bindings I use often and behave similarly are =vc-diff= and
=vc-print-log=. Try this last one, by default bound to =l=, in the
...
:CUSTOM_ID: commit
:END:
Let's say I am ready to commit, with my two files edited and one added.
I press =v= with point in the header of =vc-dir= and two windows pop up:
one where I can write the commit message, and at the bottom the list of
files included. There are a few important bindings in this window, other
than =C-c C-c= to commit:

- =C-c C-d= will show a diff of the changes, which brings us a bit
  closer to Magit's commit view.
- =C-c C-k= aborts the operation. You could just kill the buffer, but
  this command also restores the window configuration.
- =C-c C-e= will amend the last commit, bringing back its message.

**** Push and pull
:PROPERTIES:
:CUSTOM_ID: push-and-pull
:END:
I have my changes committed, and I feel ready to push them. From my
=vc-dir= window, I press =P=. It will run =git push=. Similarly =+= will
run =git pull=.\\
In both cases, you can invoke the commands with prefix argument to
modify the parameters passed to git, for example if you would like to
pull from a different branch (by editing the default command to
...
:CUSTOM_ID: branches
:END:
Speaking of branches, somewhat recently (very recently, in Emacs years)
a new prefix =C-x v b= was added for git branches, also available in
=vc-dir= under =b=:

- =b c= creates and switches to a new branch
- =b l= prints the log for a specific branch
- =b s= switches to an existing branch. By default, it adds "origin/" in
  front of the name, which gives you a detached head. So delete the
  prefix when picking a remote branch to clone it locally.

Notoriously missing is a command to simply list branches, which I added
to =b b=:
...
*** "C-x v" prefix
:PROPERTIES:
:CUSTOM_ID: c-x-v-prefix
:END:
Most of the commands described above can be invoked in any file you are
working on. For example if I am modifying index.html, I save changes and
press =C-x v == to see the diff compared to the last commit. I can also
commit that single file individually, revert it[[#fn-3][[3]]], see a log
of its commits, etc.

*** Conflicts, ediff, reset
:PROPERTIES:
:CUSTOM_ID: conflicts-ediff-reset
:END:
...
There's a =vc-ediff= command that I bound in a bunch of places, and it
does exactly what the name makes you think it does. I love ediff :)
...
*** Partial commits
:PROPERTIES:
:CUSTOM_ID: partial-commits
:END:
This was the last thing that I missed from Magit, and is also a somewhat
recent addition.\\
From a diff buffer (for example, one invoked in the current file via
=C-x v ==), you can drop a hunk using =k=, or split it using =C-c C-s=.
There are a few other bindings, check out the major mode help.

Once satisfied, /in the diff buffer/, press =C-x v v= to create a commit
with only the contents that you didn't drop.\\
I don't use this often, but it was very convenient the few times I
needed it.
...
:END:
There are many useful commands I didn't mention (to examine commit
details, retrieve a specific version of a file, or search all the
commits for a word, for example).\\
But hopefully this was enough to get someone started with the basics.

If any experienced =vc-mode= users find errors in the text, I will be
happy to [[mailto:sebastian@sebasmonia.com][receive a message]] and
update the post.

I mentioned in the intro built-in-itis, =vc-mode= was my own gateway to
start shedding third party packages for more "Emacs core". In its own
messy way, built-ins integrate and work together better than it seems.
Even if they are more idiosyncratic and have more surprising behaviours
(to the modern eye) than newer packages.
...
- Current project is determined by default-directory of current
  ...
  | *C-x p d*                    | project-find-dir (prompt directory relative to project)           |
  | *C-x p e*                    | project-eshell                                                    |
  | C-x p F                    | project-or-external-find-file                                     |
  |                            | visit a file in current project or external root                  |
  |                            | filename at point is available as part of future history          |
  |                            | with prefix, show all files in VCS directory except VCS directory |
  ...
  |                            | run next command in current project                               |
...
- xref relies on back-end provided by major mode.
  ...
  |                                      | source directory                                           |
  | 'xref-auto-jump-to-first-definition' | nil - don't jump to definition or to xref buffer           |
  |                                      | show - show the first result but keep focus on xref buffer |
  |                                      | t - similar to show but also jump to the first result      |
  |                                      | move - only move point to first result in xref buffer.     |
  |                                      | <RET> will take to definition                              |
  | *M-g M-n*                              | next-error works outside of xref buffer                    |
  | *M-g M-p*                              | previous-error works outside of xref buffer                  |
  |--------------------------------------+------------------------------------------------------------|
- xref buffer commands
  ...
  |       | display reference to current line in another window                    |
...
- Identifier search
  ...
  |                                        | with prefix, force prompt identifier                  |
  | 'xref-auto-jump-to-first-xref'         | similar to xref-auto-jump-to-first-definition         |
  | *r* OR M-x xref-query-replace-in-results | read REPLACEMENT string and rename all  identifiers   |
  |                                        | shown in xref buffer with it in all files where it is |
  |                                        | referenced                                            |
...
- /abbrev/ is a word that has been defined to expand into specified
  /expansion/
...
- Abbrev preserves case. Foo - Find outer otter vs foo - find outer otter
  ...
  | *C-x a l*                      | add-mode-abbrev (similar to C-x a g but mode specific) |
  | *C-x a i g*                    | inverse-add-global-abbrev (define expansion            |
  |                              | using abbrev word in buffer)                           |
  |                              | word before point is treated as abbrev                 |
  |                              | with prefix, define Nth word before point as abbrev    |
  |                              | with negative abbrev, use Nth word after               |
  ...
  |                              | if cnst expands to construction then recnst won't      |
  |                              | expand to reconstruction. In such cases type           |
  |                              | re M-' cnst. This adds a hyphen which is removed after |
  |                              | expansion                                              |
...
- dynamic abbrevs determines meaning of abbrevs from contents of
  buffer but should be requested explicitly
...
- After scanning current buffer M-/ searches other buffers
    |---------------------------------+----------------------------------------------------------------|
    | *M-/*                             | dabbrev-expand                                                 |
...
** Entering Dired
  ...
  |                                        |   | wildcard may appear in directory part also like /root/*/*.el                   |
  |                                        |   | on posix recursive globbing is supported so /root/**/*.el shows listing        |
  |                                        |   | with all files with .el extension descending recursively in all subdir of root |
  |                                        |   | with prefix, prompt switches for ls command                                    |
  | *C-x C-f*                                |   | find-file                                                                      |
  |                                        |   | a directory for find-file opens dired                                          |
  ...
  | *C-x C-j*                                |   | dired-jump (invoke dired on default directory)                                 |
...
** Navigation
  |----------------------------------------+---+---------------------------------------------------------------------------|
  | *<space> or n or C-n*                    |   | dired-next-line                                                           |
  | *p or C-p*                               |   | dired-previous-line                                                       |
...
** Deleting
  ...
  |                           |   | repeated d flags successive files                                 |
  |                           |   | if region is active, flags files in region for deletion           |
  |                           |   | with prefix, mark that many files for deletion                    |
  | *%d*                        |   | dired-flag-files-regexp (for deletion)                            |
  | *u*                         |   | dired-unmark                                                      |
  | *<DEL>*                     |   | dired-unmark-backward (move point to previous line and remove     |
  |                           |   | deletion flag on that line)                                       |
...
** Flagging
  ...
  | % & |   | flag file that can be recovered easily                             |
  | *% d* |   | flag all files whose name match regexp                             |
  |-----+---+--------------------------------------------------------------------|
** Visiting Files
  |--------------------------------------------+---+---------------------------------------------------------------------------|
  | *f or RET* or e                              |   | dired-find-file (visit file on current line)                              |
  ...
  | 'dired-kill-when-opening-new-dired-buffer' |   | non-nil - kill old dired buffer when selecting new directory              |
  |--------------------------------------------+---+---------------------------------------------------------------------------|
** Marks and Flags
  |------------------------------+---+----------------------------------------------------------------------------|
  ...
  | **N*                           |   | dired-number-of-marked-files (report number and size of marked files)      |
  | **                           |   | dired-mark-executables                                                     |
  | **@*                           |   | dired-mark-symlinks                                                        |
  | **/*                           |   | dired-mark-directories (exclude . and ..)                                  |
  | *s                           |   | dired-mark-subdir-files (mark all files and directories in current buffer) |
  | **C-n*                         |   | dired-next-marked-file                                                     |
  | **C-p*                         |   | dired-previous-marked-file                                                 |
  | *u*                            |   | dired-unmark                                                               |
  | *U*                            |   | dired-unmark-all-marks                                                     |
  | *t*                            |   | dired-toggle-marks (mark unmarked files and vice-verse)                    |
  | **% REGEXP <RET>* or *%m REGEXP* |   | dired-mark-files-regexp                                                    |
  |                              |   | mark files whose name match regexp                                         |
...
** Operating on files
  ...
  - With prefix N, operate on next N files including current. With -N,
    operate on N previous files
  - If files are marked with *, operate on those files
  - Otherwise operate on current file
  |--------------------------------------------------+---+------------------------------------------------------------------------------|
  | 'dired-dwim-target'                              |   | non-nil - try to guess default target directory for copy, link etc           |
  |                                                  |   | if there is other dired buffer displayed, use it as target                   |
  ...
  | dired-create-destination-dirs-on-trailing-dirsep |   | trailing / treates destination as directory,                                 |
  ...
  | *D*                                                |   | dired-do-delete                                                              |
  | *E*                                                |   | dired-do-open (using external application)                                   |
  ...
  | *Z*                                                |   | dired-do-compress (each file is compressed separately, uses gzip by default) |
...
** Shell Commands
  |----------------------------------------+---+---------------------------------------------------------------------------|
  | *!*  or *X*                                |   | dired-do-shell-command (run shell command on marked files)                |
  ...
  |                                        |   | to use * as shell wild-card, use *""                                      |
  |                                        |   | if ? is used with surrounding space, command runs once for each file      |
  |                                        |   | if neither ? or *, command runs for each file, adding filename at the end |
  |----------------------------------------+---+---------------------------------------------------------------------------|
** Transforming File Names
  ...
  | %m                                     |   | dired-mark-file-regexp                                                  |
  | %d                                     |   | dired-flag-file-regexp                                                  |
  | *%g*                                     |   | dired-mark-files-containing-regexp                                      |
  |                                        |   | regex searched inside file contents                                     |
  |----------------------------------------+---+-------------------------------------------------------------------------|
** File Comparison
  |----------------------------------------+---+-------------------------------------------------------------------------|
...
** Updating Dired Buffer
  ...
  | *l*                                      |   | dired-do-redisplay (update specified files)                           |
  | *k*                                      |   | dired-do-kill-lines                                                   |
  |                                        |   | just deletes the lines, not the files                                 |
  |                                        |   | when run on subdir heading with prefix, removes subdir listing        |
...
** Dired and Find
  ...
  |                                        |   | display output of find in dired                                       |
  | M-x find-grep-dired                    |   | like grep -R                                                          |
  |                                        |   | list files whose content match regex in dired                         |
  |                                        |   | Alternate is % g REGEXP                                               |
  |----------------------------------------+---+-----------------------------------------------------------------------|
** Editing Dired Buffer
  ...
  |                                        |   | if renamed, with subdirectory, subdir will be created                 |
  | *C-c C-c*                                |   | wdired-finish-edit (switches back to dired mode)                      |
  | 'wdired-create-parent-directories'     |   | create parent directory as needed                                     |
  | 'wdired-allow-to-change-permissions'   |   | allow changing file permission                                        |
  |----------------------------------------+---+-----------------------------------------------------------------------|
** Image Thumbnails
  |----------------------------------------+---+-----------------------------------------------------------------------|
  | *C-t d*                                  |   | image-dired-display-thumbs                                            |
  |                                        |   | mark image files and type C-t d to enter image-dired                  |
  | image-dired-thumbnail-storage          |   | location of thumbnail                                                 |
...
** Other Features
  |----------------------------------------+---+-----------------------------------------------------------------------|
  | 'dired-free-space'                     |   | display free space available                                          |
  | *+*                                      |   | dired-create-directory                                                |
  | M-x dired-create-empty-file            |   | creates an empty file. You can also use C-x C-f in dired buffer       |
  | *w*                                      |   | dired-copy-filename-as-kill (adds to kill ring)                       |
  | *W*                                      |   | browse-url-of-dired-file (open html file in browser)                  |
...
| *A s* or *l (ell)* | gnus-group-list-groups                                                      |
|                | show only groups with unread articles                                       |
| *A u* or *L*       | gnus-group-list-all-groups                                                  |
...
| *p* or <DEL>     | gnus-group-prev-unread-group                                                |
...
| *M-s M-s REGEXP RET* | gnus-summary-search-article-forward                                           |
|                    | search articles containing match for regexp                                   |
| *M-r REGEXP RET*     | gnus-summary-search-article-backward                                          |
| *q*                  | gnus-summary-exit                                                             |
...
- opening a document that can be viewed in DocView mode, Emacs
  ...
  | *K*                           | doc-view-kill-proc (only kill the conversion process)             |
  | *q*                           | quit-window                                                       |
  ...
  | *+*                           | doc-view-enlarge                                                  |
  | *-*                           | doc-view-shrink                                                   |
  | 'doc-view-scale-internally' | nil - re-render the image based on new scale                      |
  |                             | t - re-scale already rendered image                               |
  | *C-s*                         | doc-view-search                                                   |
  | *C-r*                         | doc-view-search-backward                                          |
  |                             | with prefix, both start a new search                              |
  | *C-t*                         | doc-view-show-tooltip (display search results in tooltip)         |
  | 'doc-view-cache-directory'  | location where converted images are cached                        |
  | M-x doc-view-clear-cache    | manually clear the cache                                          |
  | *g*                           | revert-buffer (force re-conversion of current buffer)             |
  |-----------------------------+-------------------------------------------------------------------|
...
|                                      | C-g sends SIGINT for sync shell command                             |
|                                      | second C-g sends SIGKILL                                            |
...
|                                      | Emacs buffer                                                        |
|                                      | with prefix, create a new subshell                                  |
...
** Serial Terminal
  |-----------------+------------------------------------------------------------|
  | M-x serial-term | ask for serial device and speed and switch to new term     |
  |                 | mode buffer                                                |
  |                 | Emacs communicates with serial device with this buffer     |
  |                 | speed can be changed interactively by clicking on modeline |
  |-----------------+------------------------------------------------------------|
...
|                         | if multiple lines have same values for the field, they keep same sort order |
|                         | such that they appear in the same relative order as in original buffer      |
...
|                         | after sort by fields, run this to reverse the order                         |
|-------------------------+-----------------------------------------------------------------------------|
* Saving Emacs Sessions
- Includes buffers and their file names, major modes, buffer positions, window
  and frame configurations and some global variables
- If Emacs starts in daemon mode, it can't use GUI, so frame
  parameters can't be restored. So the restoration should be delayed
  till first client connects by calling desktop-read in a hook added
  to server-after-make-frame-hook
- Restore of frame and windows ignores any setting from init file and
  only restores based on saved values from last session
- Remote buffer details are not saved by default
|-----------------------------------+--------------------------------------------------------|
...
| 'desktop-save-locked-desktop'     | nil - don't load desktop if locked                     |
|                                   | t - load desktop anyway                                |
|                                   | ask - ask before loading                               |
| M-x desktop-save                  | manually save desktop                                  |
| 'desktop-restore-frames'          | nil - disable save/restore of frame and window config  |
| 'frameset-filter-alist'           | list of of frame parameter that should not be restored |
| 'desktop-files-not-to-save'       | regexp of files whose buffers should not to be saved   |
| 'remote-file-name-access-timeout' | number of seconds after which restoration of remote    |
|                                   | file is stopped                                        |
| desktop-save-mode                 | setting t will save and restore automatically          |
| 'desktop-path'                    | directory where desktop is saved                       |
|                                   | If "." is included in this list, desktop is saved      |
|                                   | local to directory                                     |
| 'desktop-save'                    | t - always save                                        |
|                                   | ask - always ask                                       |
| 'desktop-restore-eager'           | number of buffers to load immediately. others          |
|                                   | are loaded lazily                                      |
| M-x desktop-clear                 | empty desktop file                                     |
| savehist-mode                     | save minibuffer history                                |
| 'savehist-file'                   | file where minibuffer history is saved                 |
| 'desktop-globals-to-save'         | list of minibuffer history variables to save as part   |
|                                   | of dsektop-save                                        |
...
- Eglot provides infrastructure and a set of commands for enriching
  the source code editing capabilities of Emacs via LSP
- The protocol allows Emacs to receive various source code services
  from the server, such as description and location of function calls,
  types of variables, class definitions, syntactic errors, etc.
- This way, Emacs doesn't need to implement the language-specific
  parsing and analysis capabilities in its own code, but is still
  capable of providing sophisticated editing features that rely on
  such capabilities, such as automatic code completion, go-to
...
- An LSP server must be first installed separately for each of the
  languages
- Eglot will then start the server as a child process of Emacs and
  communicate with it using standard input and output
|-------------------------+------------------------------------------------------|
| 'eglot-server-programs' | associates major modes with name and command line    |
|                         | arguments of the language server program             |
|                         | It is an alist of the form (MAJOR-MODE . SERVER)     |
|                         | The MAJOR-MODE can be symbol or emacs major mode or  |
|                         | list of the form (MODE :language-id ID) where ID     |
|                         | identifies language to the server                    |
|                         | The SERVER can be (PROGRAM ARGS...) or               |
|                         | (PROGRAM ARGS... :initializationOptions OPTIONS...)  |
|                         | or (HOST PORT ARGS...)                               |
|                         | or (PROGRAM ARGS... :autoport MOREARGS)              |
|                         | or FUNCTION that returns value in any of the formats |
|                         | specified above                                      |
|                         | Use add-to-list to add new server associations       |
| eglot-alternatives      | if there are more than one server for a mode, this   |
|                         | can be used to specify alternative when adding to    |
|                         | eglot-server-programs. If more than one server used  |
|                         | in alternative are installed, Eglot will prompt to   |
|                         | select one                                           |
|-------------------------+------------------------------------------------------|
*** Starting and Stopping Eglot
|----------------------+--------------------------------------------------------------------|
| M-x eglot            | start eglot                                                        |
|                      | This starts suitable language server for the visited file and      |
|                      | connects to it                                                     |
|                      | Successful connects is indicated in mode-line like [eglot:PROJECT] |
|                      | A single Eglot session serves all buffers in same project and with |
|                      | same major mode                                                    |
|                      | Need to invoke only once per project                               |
| Autostart            | (add-hook 'foo-mode-hook 'eglot-ensure)                            |
|                      | Automatically start eglot when major mode is foo-mode              |
| M-x eglot-shutdown   | shutdown language server manually                                  |
|                      | prompts for server to shutdown if there are more than one          |
| 'eglot-autoshutdown' | t - automatically shutdown server process when last buffer using   |
|                      | it is killed                                                       |
|                      | nil - dont' kill so that visiting another file automatically       |
|                      | activates Eglot so that server is started once per session         |
|----------------------+--------------------------------------------------------------------|
** Using Eglot
*** Features
- At point documentation via Eldoc
- On the fly diagnostics annotations via Flymake
- Finding definitions and identifiers via Xref
- Buffer navigation by function name, class name etc via Imenu
- Completion of symbol at point using completion-at-point
- Reformatting of source code similar to eglot-format
- Support of snippet completion if yasnippet is installed
- Formatted at point-documentation if markdown-mode is installed
- Renaming of variables or code actions etc are also supported
*** Projects and Eglot
- Eglot starts at project root as CWD so the server analyzes entire
  project files
- Project specific information can be passed with
  eglot-workspace-configuration
*** Commands
|-----------------------------------------+-------------------------------------------------------------|
| M-x eglot                               | starts eglot server if not already running for the project  |
|                                         | with prefix, prompt for server program                      |
|                                         | with two prefix, also prompt major mode                     |
| M-x eglot-reconnect                     | shutdown current connection to server and reconnects        |
| M-x eglot-shutdown                      | shutdown language server                                    |
|                                         | with prefix, don't kill eglot buffer                        |
| M-x eglot-shutdown-all                  | shutdown all active language server                         |
|                                         | with prefix, don't kill elgot buffers                       |
| M-x eglot-rename                        | rename symbol at point to another name and modifies all     |
|                                         | files in project to do the refactoring of name              |
| M-x eglot-format                        | reformat active region or entire buffer                     |
| M-x eglot-format-buffer                 | reformat entire buffer                                      |
| M-x eglot-code-actions                  | Allows import reorganizing, fixing diagnostic error         |
|                                         | reported by flymake etc. There are also specific version of |
|                                         | this command like eglot-code-action-organize-imports etc    |
| M-x eglot-inlay-hints-mode              | toggles LSP inlay hints (text annotations) for current      |
|                                         | buffer. These include readability improvements              |
| M-x eglot-events-buffer                 | open events buffer used for language server communication   |
| M-x eglot-stderr-buffer                 | open debug buffer created by language server error stream   |
| M-x eglot-forget-pending-continuations  | forget pending requests for server of current buffer        |
| M-x eglot-signal-didChangeConfiguration | update language server configuration to current value of    |
|                                         | eglot-workspace-configuration                               |
| M-x eglot-clear-status                  | clear last JSONRPC error for the server for current buffer  |
|-----------------------------------------+-------------------------------------------------------------|
*** Variables
|-----------------------------------+--------------------------------------------------------------------------------|
| eglot-autoreconnect               | attempt to reconnect automatically if server process terminates                |
|                                   | number - minimum seconds previous connection was up for it to                  |
|                                   | reconnect                                                                      |
|                                   | t - always reconnect automatically                                             |
|                                   | nil - never reconnect                                                          |
| eglot-connect-timeout             | number of seconds after which connection attempt times out                     |
|                                   | nil - never timeout                                                            |
| eglot-sync-connect                | number - max seconds up to which emacs can be blocked when                     |
|                                   | connecting after which wait in background                                      |
|                                   | t - block for entire waiting period                                            |
|                                   | 0 or nil - don't block                                                         |
| eglot-event-buffer-config         | size and format of eglot event buffer                                          |
|                                   | default: (:size 2000000 :format full)                                          |
| eglot-autoshutdown                | t - shutdown when last buffer managed is killed                                |
|                                   | nil - don't shutdown                                                           |
| eglot-confirm-server-edits        | summary - ask confirmation before performing edits proposed by language server |
|                                   | diff - show a diff buffer so user can apply changes individually               |
|                                   | nil - apply changes without confirmation                                       |
|                                   | (COMMAND . ACTION) - where                                                     |
|                                   | COMMAND is command like eglot-rename or t for catch all case                   |
|                                   | ACTION is one of nil, summary or diff.                                         |
| eglot-ignored-server-capabilities | list of language server capabilities eglot should ignore                       |
|                                   | nil - use all capabilities                                                     |
| eglot-extend-to-xref              | t - M-. can take you out of project like system header                         |
|                                   | nil - scoped to current project                                                |
| eglot-mode-map                    | keymap in eglot mode                                                           |
|                                   | only C-h . is re-mapped to eldoc-doc-buffer by default                         |
|-----------------------------------+--------------------------------------------------------------------------------|
** Customizing
|-------------------------------+----------------------------------------------------------------|
| eglot-server-programs'        | associate language server to major mode                        |
| eglot-strict-mode             | nil - be lenient about non-conforming servers                  |
| eglot-server-initialized-hook | run after the server is initialized but before connection is   |
|                               | made. Each function receives eglot-server-instance as argument |
| eglot-connect-hook            | run after connection to server is established                  |
| eglot-managed-mode-hook       | run after eglot starts or stops managing a buffer              |
| eglot-stay-out-of             | list of Emacs features Eglot should not try to manage like     |
|                               | flymake                                                        |
| eglot-report-progress         | t - show progress report using Emacs progress reporting        |
|                               | facility mainly echo area                                      |
|-------------------------------+----------------------------------------------------------------|
** Advanced Server Configuration
- There are two types of server configurations:
  - Project specific
  - User specific, applied for all projects
- Not all servers allow both kind of configurations nor that user
  options are copied to project options
- Some servers read settings from config file like
  ~/.config/pycodestyle for pylsp and ~/.config/clangd/config.yaml for
  clangd for user config. Similarly .clangd for project config
*** Project Specific
- Use directory-local variable eglot-workspace-configuration. In Emacs
  this is achieved using .dir-locals.el file in project root
- The variable is a plist of the form (:SERVER1 PLIST1 :SERVER2 PLIST2
  ...)
- /M-x eglot-show-workspace-configuration/ to inspect and debug the
  value of this variable
- The configuration options are set as per server documentation.
- Given following options from server documentation
  #+begin_quote
  ...
  #+end_quote
- Corresponding .dir-locals.el entry looks like
  #+begin_quote
     ((nil
       . ((eglot-workspace-configuration
  ...
  #+end_quote
- Each server only takes section intended for that server. The setting
  applies to all buffers inside the project
- Alternate way to specify the same is using major modes:
  #+begin_quote
     ((python-base-mode
       . ((eglot-workspace-configuration
	   . (:pylsp (:plugins (:jedi_completion (:include_params t
	  ...
	  (indent-tabs-mode . t))))
  #+end_quote
- To set some configuration only to sub-directory of project
  #+begin_quote
     ((python-base-mode
       . ((eglot-workspace-configuration
	   . (:pylsp (:plugins (:jedi_completion (:include_params t
...
- Finally, if one needs to determine the workspace configuration based
  on some dynamic context, ‘eglot-workspace-configuration’ can be set
  to a function.  The function is called with the ‘eglot-lsp-server’
  instance of the connected server (if any) and with
  ‘default-directory’ set to the root of the project.  The function
...
- Python venv example
  ...
  ((nil
   . ((eglot-workspace-configuration
      ...
      (eval
       . (let* ((venv-root (expand-file-name ".venv" default-directory))
		(venv-bin  (expand-file-name "bin" venv-root))
		(python    (expand-file-name "python" venv-bin))
		(path      (getenv "PATH")))
	   (when (file-directory-p venv-bin) ;; Update PATH (avoid duplication)
	     (unless (and path (string-prefix-p venv-bin path))
	      (setq process-environment
	       (cons (concat "PATH=" (concat venv-bin ":" path))
		(seq-remove (lambda (s) (string-prefix-p "PATH=" s)) process-environment))))
	     (when (file-executable-p python)
	      (setq-local python-shell-interpreter python))
	      (add-to-list 'exec-path venv-bin)))))))
  #+end_quote
- *For tramp pylsp, add path of pylsp binary in .profile*
  #+begin_quote
  if [ -d "$HOME/.cache/lsp/pylsp/bin" ] ; then
      PATH="$HOME/.cache/lsp/pylsp/bin:$PATH"
  fi
  #+end_quote
*** User Specific Configuration
- A common way is to pass the command line option to server invocation
  ...
  #+end_quote
- Another way is to pass the options via :initializationOptions
  #+begin_quote
     (with-eval-after-load 'eglot
       (add-to-list 'eglot-server-programs
  ...
  #+end_quote
  This is equivalent of JSON option {"compilationDatabasePath": "/tmp"}
- Third way is to set global eglot-workspace-configuration
  #+begin_quote
     (setq-default eglot-workspace-configuration
		   '(:pylsp (:plugins (:jedi_completion (:include_params t
		     ...
		     :gopls (:usePlaceholders t)))
  #+end_quote
  The global value is overridden by project specific values
...
#+end_src
** Custom Venv Handling
#+begin_src emacs-lisp :tangle no
  (defvar-local my/env-script ".venv"
  ...
  for the current buffer/process. No-op if `my-python-venv-path` is nil."
    (interactive)
    (let* ((venv-root (expand-file-name my/env-script
				      (locate-dominating-file default-directory my/env-script)))
	   (venv-bin  (expand-file-name "bin" venv-root))
	 ...
	 (newpenv   (cons (concat "VIRTUAL_ENV_PROMPT=" my/env-script)
	...
	(setq-local exec-path (nconc (split-string newpath path-separator)
				   (list exec-directory))))))
  ;; (defvar my/env-script ".venv/bin/activate"
  ;;   "Script to be executed to load environment variables")
  ;; (defun my/env-script-activate ()
  ;;   (interactive)
  ;;   (let* ((outbuf (with-current-buffer (get-buffer-create " *my-env-outbuf*")
  ;;		   (erase-buffer)
  ;;		   (current-buffer)))
  ;;	 (errbuf (with-current-buffer (get-buffer-create " *my-env-errbuf*")
  ;;		   (erase-buffer)
  ;;		   (current-buffer)))
  ;;	 (proc (make-process
  ;;		:name "my-venv"
  ...
  ;;     (let ((env-output (with-current-buffer outbuf (buffer-string)))
  ;;	  (penv nil))
  ...
  (defun my/env-script-activate-inherit (func &rest args)
  ...
  ;;           (apply func args))
  ;;       (apply func args))))
  (add-hook 'hack-local-variables-hook #'my/env-script-activate)
  (add-hook 'comint-mode-hook #'my/env-script-activate)
  (add-hook 'eshell-mode-hook #'my/env-script-activate)
  (add-hook 'org-mode-hook #'my/env-script-activate)
  (add-hook 'org-src-mode-hook #'my/env-script-activate)
  (advice-add 'eshell :around #'my/env-script-activate-inherit)
  (advice-add 'shell :around #'my/env-script-activate-inherit)
#+end_src
** Configuration
#+begin_src emacs-lisp
  (use-package python
...
|-------+------------------------------------------------------------------------|
| *g*     | refresh buffer                                                         |
|       | with prefix, show internal buffers                                     |
| *m*     | mark buffer                                                            |
...
| M-UP    | org-metaup                                                       |
|         | move entire entry(including body and subtree) up                 |
| M-DOWN  | org-metadown                                                     |
| M-LEFT  | org-metaleft                                                     |
|         | promote a heading                                                |
...
|                             | open a separate window for source editing etc |
| /emphasized/                  |                                               |
| *bold*                        |                                               |
| _underlined_                  |                                               |
| =teletype font=               |                                               |
...
#+begin_src emacs-lisp :tangle no
  (use-package gptel
    ...
    (gptel-use-curl nil)
    (gptel-stream t)
    (gptel-use-context 'system)		; additional context goes to system prompt
    (gptel-use-tools t)
    (gptel-default-mode 'org-mode)
    (gptel-highlight-methods 'face)	; highlight with difference face
    (gptel-track-response t)		   ; enable text properties to distinguish user message and llm response
    (gptel-track-media t)			; support multimedia
    (gptel-confirm-tool-calls t)		; always ask
    (gptel-include-tool-results t)
    (gptel-use-header-line t)
    (gptel-display-buffer-action '(display-buffer-in-side-window
    				 (body-function . select-window)
    				 (side . right)
    				 (window-width . 0.4)))
    (gptel-org-branching-context t)	   ; each org level is a separate conversation branch
    (gptel-org-validate-link 'always)
    (gptel-rewrite-default-action 'dispatch) ; dispatch transient on rewrite
    
    (gptel-highlight-mode t)
    (gptel-context-restrict-to-project-files t)
    (gptel-include-reasoning 'ignore)	; include thinking block in response but ignore on next turn
    (gptel-expert-commands t)		   ; advanced options in menu
    :bind
    ...
    (setopt gptel-model 'openrouter/free
  	  gptel-backend
  ...
  (use-package gptel-agent
    ...
    :defer t
    :custom
    (gptel-agent-dirs (list (expand-file-name "agents"(file-name-directory user-init-file))))
    :bind
    (:map my/gptel-prefix-map
  	("A" . gptel-agent)
  	)
...
#+begin_src emacs-lisp :tangle no
  (use-package agent-shell
    ...
    (:map my/ai-prefix-map
      	...
      	("K" . agent-shell-remove-pending-request)
      	("L" . agent-shell-toggle-logging)
      	("Q" . agent-shell-queue-request)
      	("R" . agent-shell-resume-pending-requests)
...
#+begin_src emacs-lisp :tangle no
  (use-package ellama
    ...
    :defer t
    :preface
    (defun my/ellama-display-action (buffer &optional alist)
      (let ((name (buffer-name (get-buffer buffer))))
        (cond
         ;; Context buffer (and similar) go to the bottom
         ((string-match-p "\\*ellama-context" name)
       	(display-buffer-in-side-window
       	 buffer
       	 '((body-function . select-window)
             (side . bottom)
             (window-height . 0.3))))
         ;; Everything else (chat buffers) go to the right
         (t
       	(display-buffer-in-side-window
       	 buffer
       	 '((body-function . select-window)
    ...
    :bind-keymap
    ("C-c a" . ellama-command-map)
    :bind
    (:map ellama-command-map
       	("c p" . ellama-plan-and-act)
       	("s C" . ellama-session-compact-current)
    ...
    (ellama-max-tokens 10000)
    (ellama-auto-scroll nil)
    (ellama-fill-paragraphs (text-mode))
    ...
    ;; Display and Context
    (ellama-chat-display-action-function #'my/ellama-display-action)
    (ellama-image-context-default-scope 'ephemeral)
    (ellama-always-show-chain-steps t)
    (ellama-session-remove-reasoning t)
    (ellama-session-hide-org-quotes t)
    (ellama-output-remove-reasoning t)
    (ellama-context-manage-display-action-function #'my/ellama-display-action)
    (ellama-context-preview-element-display-action-function #'my/ellama-display-action)
    ...
    (ellama-tools-allowed
     '(ellama-tools-directory-tree-tool
       ellama-tools-read-file-tool
       ellama-tools-grep-tool		; grep files in directory
       ellama-tools-grep-in-file-tool	; grep a file
       ellama-tools-lines-range-tool
       ellama-tools-count-lines-tool	; get line count
       ellama-tools-project-root-tool
       ellama-tools-now-tool		; current timestamp
       ellama-tools-task-tool		; delegate task to sub-agent
       ellama-tools-task-from-template-tool ; delegate task rendered to subagent
       ))
    (ellama-tools-dlp-enabled t)		; enable DLP
    (ellama-tools-dlp-mode 'enforce)	; enforce by default
    (ellama-tools-dlp-log-targets '(memory file)) ; log to memory and file
    (ellama-tools-dlp-audit-log-file (expand-file-name "ellama-dlp.jsonl" user-emacs-directory))
    :config
    (setopt openrouter
    ...
    (setopt opencode
            (make-llm-openai-compatible
             :chat-model "deepseek-v4-flash-free"
             :url "https://opencode.ai/zen/v1"))
    (setopt ellama-providers
            `(("OpenRouter" . ,openrouter)
              ("OpenCode" . ,opencode)))
    (setq ellama-coding-provider opencode		 ; for coding
       	ellama-summarization-provider openrouter ; also used for compaction
       	ellama-provider openrouter ; catch all
       	ellama-tools-dlp-regex-rules
    	'((:id "deny-env-files"
    	       :pattern "\\.env\\(\\.\\|$\\)"
    	       :directions (input)
    	       :tools ("read_file" "write_file" "edit_file"
    		       "append_file" "prepend_file" "move_file"
    		       "directory_tree" "grep_in_file" "grep"
    		       "shell_command")
    	       :severity high)
    	  (:id "deny-keys-folder"
    	       :pattern "\\(^\\|/\\)keys\\(/\\|$\\)"
...
#+end_src
* Markdown Mode
** Insertion Commands (C-c C-s prefix)
| Key       | Command                             | Description                     |
|-----------+-------------------------------------+---------------------------------|
| C-c C-s b | markdown-insert-bold                | Insert bold text or bold region |
| C-c C-s i | markdown-insert-italic              | Insert italic text              |
| C-c C-s c | markdown-insert-code                | Insert inline code              |
| C-c C-s C | markdown-insert-gfm-code-block      | Insert fenced code block        |
| C-c C-s l | markdown-insert-link                | Insert inline link              |
| C-c C-s L | markdown-insert-reference-link-dwim | Insert reference-style link     |
| C-c C-s u | markdown-insert-uri                 | Insert URL                      |
| C-c C-s f | markdown-insert-footnote            | Insert footnote                 |
| C-c C-s w | markdown-insert-wiki-link           | Insert wiki link                |
| C-c C-s i | markdown-insert-image               | Insert image                    |
| C-c C-s h | markdown-insert-header-dwim         | Insert/update ATX header        |
| C-c C-s H | markdown-insert-header-setext-dwim  | Insert Setext header            |
| C-c C-s - | markdown-insert-hr                  | Insert horizontal rule          |
| C-c C-s q | markdown-insert-blockquote          | Insert blockquote               |
| C-c C-s Q | markdown-blockquote-region          | Convert region to blockquote    |
| C-c C-s p | markdown-insert-pre                 | Insert preformatted block       |
| C-c C-s t | markdown-insert-table               | Insert table                    |
| C-c C-s T | markdown-table-convert-region       | Convert region to table         |
| C-c C-s n | markdown-cleanup-list-numbers       | Fix ordered list numbering      |
** Toggle Commands (C-c C-x prefix)
| Key         | Command                                      | Description                                |
|-------------+----------------------------------------------+--------------------------------------------|
| C-c C-x C-l | markdown-toggle-url-hiding                   | Toggle visibility of URLs                  |
| C-c C-x C-i | markdown-toggle-inline-images                | Toggle display of inline images            |
| C-c C-x C-e | markdown-toggle-math                         | Toggle math support                        |
| C-c C-x C-m | markdown-toggle-markup-hiding                | Toggle hiding of markup syntax             |
| C-c C-x C-f | markdown-toggle-fontify-code-blocks-natively | Toggle native fontification of code blocks |
|             |                                              | inside code blocks                         |
** Navigation Commands
| Key     | Command                           | Description               |
|---------+-----------------------------------+---------------------------|
| C-c C-n | markdown-next-visible-heading     | Jump to next heading      |
| C-c C-p | markdown-previous-visible-heading | Jump to previous heading  |
| C-c C-f | markdown-forward-paragraph        | Move forward paragraph    |
| C-c C-b | markdown-backward-paragraph       | Move backward paragraph   |
| C-c C-u | markdown-backward-up-heading      | Move up heading hierarchy |
** Editing / Structure Commands
| Key         | Command                      | Description                      |
|-------------+------------------------------+----------------------------------|
| C-c C--     | markdown-promote             | Promote heading or list          |
| C-c C-=     | markdown-demote              | Demote heading or list           |
| C-c <left>  | markdown-promote             | Promote element                  |
| C-c <right> | markdown-demote              | Demote element                   |
| M-<left>    | markdown-promote-subtree     | Promote subtree                  |
| M-<right>   | markdown-demote-subtree      | Demote subtree                   |
| M-<up>      | markdown-move-subtree-up     | Move subtree up                  |
| M-<down>    | markdown-move-subtree-down   | Move subtree down                |
| C-c C-k     | markdown-kill-thing-at-point | Remove markup but keep text      |
| C-c C-w     | markdown-kill-ring-save      | Copy Markdown element            |
| C-c C-d     | markdown-do                  | Perform action on thing at point |
** Completion & Cycling
| Key     | Command           | Description             |
|---------+-------------------+-------------------------|
| TAB     | markdown-cycle    | Cycle visibility / fold |
| S-TAB   | markdown-shifttab | Cycle global visibility |
| C-c C-] | markdown-complete | Complete markup         |
** Export / Preview Commands
| Key       | Command                     | Description                 |
|-----------+-----------------------------+-----------------------------|
| C-c C-c m | markdown-command            | Run Markdown processor      |
| C-c C-c p | markdown-preview            | Preview Markdown in browser |
| C-c C-c e | markdown-export             | Export to file              |
| C-c C-c v | markdown-export-and-preview | View exported output        |
** Following Links
| Key     | Command                        | Description            |
|---------+--------------------------------+------------------------|
| C-c C-o | markdown-follow-thing-at-point | Open link or reference |
| RET     | markdown-follow-thing-at-point | Follow link at point   |
| mouse-1 | markdown-follow-thing-at-point | Click to follow link   |
** Miscellaneous
| Key       | Command                  | Description                |
|-----------+--------------------------+----------------------------|
| C-c C-a l | markdown-insert-link     | Alternative link insertion |
| C-c C-a u | markdown-insert-uri      | Insert URL                 |
| C-c C-a f | markdown-insert-footnote | Insert footnote            |
** Notes
- Many commands operate on the *active region* if selected.
- Otherwise, they act on the *element at point*.
- Uppercase variants often force operation on region.
- Some bindings may vary depending on configuration or extensions.
** Configuration
#+begin_src emacs-lisp
...
** Paredit
    ...
    (use-package paredit
      :ensure t
      :defer t
      :hook (emacs-lisp-mode ielm-mode lisp-mode lisp-interaction-mode scheme-mode eval-expression-minibuffer-setup)
      :bind
      (:map paredit-mode-map
    	...
    	;; ("C-M-a" . paredit-forward-up)
    	;; ("C-M-e" . paredit-backward-down)
    	;; Splicing
    	("M-<up>" . paredit-splice-sexp-killing-backward)
    	("M-<down>" . paredit-splice-sexp-killing-forward)
    	("M-s" . nil)			; don't overwrite M-s
    	;; Split and Join
    	("M-S" . paredit-split-sexp)
    	("M-J" . paredit-join-sexp)
    	;; kill
    	("C-k" . paredit-kill)
    	;; Unmap bindings that are useful elsewhere
    	("M-?" . nil)
    	("M-r" . nil)
    	("C-M-n" . nil)
    	("C-M-p" . nil)
...
* Untangle the file
# Local Variables:
# eval:(add-hook 'after-save-hook (lambda () (org-babel-tangle-file buffer-file-name (concat (file-name-sans-extension buffer-file-name) ".el")) (message "Untangled the file") (when (native-comp-available-p) (native-compile-async (concat (file-name-sans-extension buffer-file-name) ".el")))) nil t)
# End:
