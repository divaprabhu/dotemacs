...
#+begin_src emacs-lisp
  ...
  (setq completion-auto-help 'lazy) ;; show completion list buffer on second tab
  (setq completion-auto-select nil) ;; don't switch to completion list buffer when displayed
  (setq completion-cycle-threshold 5) ;; cycle through completion candidates if count is less than 5
...
n | *M-% STRING RET NEWSTRING RET*                  |                         | query-replace                                                    |
...
- xref relies on back-end provided by major mode. Emacs includes
  ...
  | *C-M-,*             | xref-go-forward                                                     |
  |                     | Emacs 29 only (or install xref from elpa)                           |
...
#+begin_src
  ...
  (define-key 'my-prefix-map (kbd "<SPC>") 'just-one-space)
